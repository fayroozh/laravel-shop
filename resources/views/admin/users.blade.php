...
<div id="editUserModal{{ $user->id }}" class="modal">
    <div class="modal-content">
        ...
        <form method="POST" action="{{ route('admin.users.update', $user) }}">
            ...
            <div class="form-group">
                <label>Name</label>
                <input id="editName{{ $user->id }}" type="text" name="name" class="form-control" required value="{{ old('name', $user->name) }}">
            </div>
            <div class="form-group">
                <label>Email</label>
                <input id="editEmail{{ $user->id }}" type="email" name="email" class="form-control" required value="{{ old('email', $user->email) }}">
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
                ...
                <select id="editRoleSelect{{ $user->id }}" name="role_id" class="form-control" required>
                    ...
                    @foreach($roles as $role)
                        <option value="{{ $role->id }}" 
                            @if($role->id == $user->roles->pluck('id')->first()) selected @endif
                            @if($role->name === 'employee') data-is-employee="true" @endif>
                            {{ $role->display_name }}
                        </option>
                    @endforeach
            ...
            <div id="editEmployeeFields{{ $user->id }}" style="display: none;">
                <div class="form-group">
                    <label>Position</label>
                    <input id="editPosition{{ $user->id }}" type="text" name="position" class="form-control" value="{{ old('position', $user->position ?? '') }}">
                </div>
                <div class="form-group">
                    <label>Department</label>
                    <input id="editDepartment{{ $user->id }}" type="text" name="department" class="form-control" value="{{ old('department', $user->department ?? '') }}">
                </div>
                <div class="form-group">
                    <label>Phone</label>
                    <input id="editPhone{{ $user->id }}" type="text" name="phone" class="form-control" value="{{ old('phone', $user->phone ?? '') }}">
                </div>
            </div>

            <div class="form-actions">
                <button type="button" onclick="closeModal('editUserModal{{ $user->id }}')" class="btn-cancel">Cancel</button>
...
<div id="deleteUserModal{{ $user->id }}" class="modal">
    <div class="modal-content">
        ...
        <form method="POST" action="{{ route('admin.users.destroy', $user) }}">
            ...
            @method('DELETE')
            <div class="form-actions">
                <button type="button" onclick="closeModal('deleteUserModal{{ $user->id }}')" class="btn-cancel">Cancel</button>
