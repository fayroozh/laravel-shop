...
<head>
    ...
    <style>
        ...
        /* Status Colors */
        .status-high { color: var(--success-color); }
        .status-normal { color: var(--accent-color); }
        .status-low { color: var(--danger-color); }

        /* Type Colors */
        .type-new { color: var(--accent-color); }
        .type-regular { color: var(--success-color); }
        .type-vip { color: var(--warning-color); }
        ...
        .dashboard-header {
            ...
            margin-left: 20px; /* Add space from sidebar */
        }
        ...
        .content {
            margin-left: 270px; /* Add more space from sidebar */
            padding: 20px;
            min-height: 100vh;
            background: #f8f9fa;
            flex: 1;
        ...
        .sidebar h2 {
            ...
            white-space: nowrap; /* Keep "Admin Dashboard" on one line */
            color: white;
        ...
        .stat-card.products::before { background: linear-gradient(90deg, #3498db, #2980b9); }
        .stat-card.orders::before { background: linear-gradient(90deg, #e74c3c, #c0392b); }
        .stat-card.employees::before { background: linear-gradient(90deg, #2ecc71, #27ae60); }
        .stat-card.suppliers::before { background: linear-gradient(90deg, #f39c12, #e67e22); }
        .stat-card.feedback::before { background: linear-gradient(90deg, #9b59b6, #8e44ad); }
        .stat-card.revenue::before { background: linear-gradient(90deg, #1abc9c, #16a085); }
...
.report-card:hover {
    ...
    box-shadow: 0 8px 25px rgba(0,0,0,0.15);
        ...
        .btn-edit:hover, .btn-delete:hover, .btn-submit:hover, .btn-cancel:hover, .btn-danger:hover {
            transform: translateY(-3px);
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Show/hide notification dropdown when clicking the button
        ...
        window.addEventListener('click', function(event) {
            if (!event.target.matches('.notification-btn') && !event.target.closest('.notification-dropdown-content')) {
        ...
        fetch('{{ route("admin.notifications.unreadCount") }}')
            ...
            .then(data => {
                ...
                if (countElement) {
                    ...
                    // Show/hide counter based on notification presence
                    if (data.count > 0) {
    ...
    function loadLatestNotifications() {
            ...
            .then(notifications => {
                ...
                if (container) {
                    ...
                    notifications.forEach(notification => {
                        let icon = '🔔';
                        let title = notification.data.title || 'System Notification';
                        let link = '{{ route("admin.notifications.index") }}';
        ...
        function setThemeMode(mode) {
            if (mode === 'dark' && !document.body.classList.contains('dark-theme')) {
                toggleTheme();
            } else if (mode === 'light' && document.body.classList.contains('dark-theme')) {
                toggleTheme();
            }
        }
        ...
        function saveThemeSettings() {
            ...
            localStorage.setItem('primary-color', document.getElementById('primaryColor').value);
...
<body>
    <!-- Sidebar with user management link -->
    <div class="sidebar">
        <h2>👑Admin Dashboard</h2>
        <a href="{{ route('admin.dashboard') }}">📊 <span>Statistics</span></a>
        <a href="{{ route('admin.products') }}">📦 <span>Products</span></a>
        <a href="{{ route('admin.categories') }}">🗂️ <span>Categories</span></a>
        <a href="{{ route('admin.employees') }}">👥 <span>Employees</span></a>
        <a href="{{ route('admin.suppliers') }}">🏭 <span>Suppliers</span></a>
        <a href="{{ route('admin.orders') }}">🛒 <span>Orders</span></a>
        <a href="{{ route('admin.feedback') }}">📝 <span>Feedback</span></a>
        <a href="{{ route('admin.users') }}">👤 <span>Users</span></a>
        <a href="{{ route('admin.notifications.index') }}">🔔 <span>Notifications</span></a>
        <a href="{{ route('admin.activities') }}">🕒 <span>Activities</span></a>
        <a href="{{ route('admin.reports.index') }}">📈 <span>Reports</span></a>
        <a href="{{ route('admin.roles.index') }}">🔐 <span>Roles</span></a>
        <div class="theme-toggle-container">
            <button onclick="toggleTheme()" class="btn-theme" id="themeToggle">🌙 <span>Dark Mode</span></button>
        </div>
    </div>
    
    <!-- Add notification button in the main interface -->
    <div class="top-bar">
...
@push('scripts')
<script>
// SPA-like navigation to prevent page reload
